...
val trecFeb = Source.fromFile("trecFeb").getLines
val trecMarch = Source.fromFile("trecMarch").getLines
...
val output = trecTweets_all.map(tweet => {
 val this_set = tweet._2.filter(t => (t \ "delete") != JNothing)
 .map(t => {
  implicit val formats = DefaultFormats
  (t \ "delete" \ "status" \ "id").extract[String]
 }).toSet
 val intersection_set = original_ids.intersection(this_set)
 original_ids = original_ids -- intersection_set
 (tweet._1, original_ids.size())
})